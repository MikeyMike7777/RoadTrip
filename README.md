1. **Road-trip-api: Backend Setup**
	...
	- Setup Project in IntelliJ
		...
		- Configure Lombok
			...
			---
			- If you can’t find the “IntelliJ IDEA > Preferences > Plugins” then just go to “File > Settings > Plugins” and search for the plugin.
			- For “IntelliJ IDEA > Preferences > Build, Execution, Deployment”, just go to “File > Settings > Build, Execution, Deployment”, and click the dropdown icon beside “Compiler”. It will show the `Annotation Processors` option.
...
3. **CI/CD Setup**
	...
	- setup a GitLab runner: The GitLab runner waits for commits to be made to the branch it is configured to watch and runs a user-defined set of commands when one occurs. This runner must be linked to your repository first.
		...
		- register the GitLab runner (for Linux/Debian/Ubuntu/Mint; _The runner executor should be 'shell'_):
			- Go to _GitLab Project > Settings > CI/CD > Runners_ section and follow the Project runners instructions
			- Or register it using the following instructions: https://docs.gitlab.com/runner/register/#one-line-registration-command
			- You can find the _PROJECT_REGISTRATION_TOKEN_ at _GitLab Project > Settings > CI/CD > Runners_ section
		- run the following command, giving your gitlab runner permissions to run docker commands: `sudo usermod -aG docker gitlab-runner` 
			- to check if the gitlab-runner has access to Docker run: `sudo -u gitlab-runner -H docker info`
	...
	- Write a gitlab-cli.yml file.
		...
		- To learn more about how to create and run docker image from gitlab-ci.yml file, see: https://docs.gitlab.com/ee/ci/docker/using_docker_build.html
